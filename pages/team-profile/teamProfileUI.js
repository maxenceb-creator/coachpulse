(function(global){
  ...
  function renderResults(summary){
    return `<section class="team-grid">
      <article class="panel wide"><h2>Historique des matchs</h2>${table(['Date','Adversaire','Lieu','Type','Compétition','Score','Résultat','Système'], summary.matches.map(m => `<tr><td>${esc(m.date || '-')}</td><td>${esc(m.opponent)}</td><td>${m.venue === 'home' ? 'Domicile' : 'Extérieur'}</td><td>${esc(Filters.matchTypeLabel(m))}</td><td>${esc(m.competition)}</td><td><b>${m.goalsFor}-${m.goalsAgainst}</b></td><td><span class="result ${m.resultCode}">${esc(m.resultLabel)}</span></td><td>${esc(m.system || '-')}</td></tr>`), 'Aucun match sur cette période.')}</article>
