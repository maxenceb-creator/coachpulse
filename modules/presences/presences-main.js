...
function sessionSubCategories(session){return [...new Set((session?.categories||[]).flatMap(c=>CATEGORY_MAP[c]||[c]).filter(Boolean))]}
function playerMatchesSession(p,session){const subs=sessionSubCategories(session);return !subs.length || subs.includes(playerSubCategory(p))}
...
state.sessions=(state.sessions||[]).map(s=>({...s,start:s.start||'',end:s.end||'',categories:s.categories||inferCats(s),entries:Object.fromEntries(Object.entries(s.entries||{}).map(([pid,e])=>[pid,{...e,code:normalizeCode(e.code)}]))}));
...
function categoryChecks(containerId,defaults=[]){document.getElementById(containerId).innerHTML=TRAINING_CATEGORIES.map(c=>`<label class="check"><input type="checkbox" value="${c}" ${defaults.includes(c)?'checked':''}>${c}</label>`).join('')}
function selectedChecks(containerId){return [...document.querySelectorAll(`#${containerId} input:checked`)].map(i=>i.value)}
document.getElementById('sessionDate').value=today();document.getElementById('calDate').value=today();categoryChecks('calCategories',['U12-U13']);categoryChecks('sessionCategories',['U12-U13']);setDefaultRecurringUntil();
function createSessionFromCalendar(){const date=calDate.value||today(),start=calStart.value,end=calEnd.value,duration=durationFromTimes(start,end),type=calType.value,theme=calTheme.value.trim(),categories=selectedChecks('calCategories');createSessionObj({date,start,end,duration,type,theme,categories:categories.length?categories:['U12-U13']});showViewById('saisie')}
function createSession(){const date=sessionDate.value||today(),start=sessionStart.value,end=sessionEnd.value,duration=Number(sessionDuration.value||durationFromTimes(start,end)),type=sessionType.value,theme=sessionTheme.value.trim(),categories=selectedChecks('sessionCategories');createSessionObj({date,start,end,duration,type,theme,categories:categories.length?categories:['U12-U13']})}
...
function sessionExistsOn(date,start,categories){return state.sessions.some(s=>s.date===date&&s.start===start&&sameCats(s.categories,categories))}
function createRecurringAdvanced(){const startDate=calDate.value||today();const until=recurUntil?.value||startDate;const freq=recurFreq?.value||'weekly';const maxCount=Math.max(1,Math.min(120,Number(recurMax?.value||40)));const categories=selectedChecks('calCategories');const cats=categories.length?categories:['U12-U13'];let cur=new Date(startDate+'T00:00:00'),end=new Date(until+'T00:00:00');if(end<cur){alert('La date de fin doit être après la date de départ.');return}let created=0,skipped=0,guard=0;while(cur<=end&&created<maxCount&&guard<160){const ds=cur.toISOString().slice(0,10);if(sessionExistsOn(ds,calStart.value,cats)){skipped++}else{createSessionObj({date:ds,start:calStart.value,end:calEnd.value,duration:durationFromTimes(calStart.value,calEnd.value),type:calType.value,theme:calTheme.value.trim(),categories:cats,recurrence:freq});created++}cur=freq==='monthly'?addMonths(cur,1):addDays(cur,freq==='biweekly'?14:7);guard++}alert(created+' séance(s) créée(s) · '+skipped+' doublon(s) ignoré(s).');renderCalendar();renderAll()}
...
function renderCatChips(){catChips.innerHTML=['Toutes',...TRAINING_CATEGORIES].map(c=>`<button class="chip ${activeCat===c?'active':''}" onclick="activeCat='${c}';renderCalendar()">${c}</button>`).join('')}
...
function monthSessions(){const y=viewDate.getFullYear(),m=viewDate.getMonth();return state.sessions.filter(s=>{const d=new Date(s.date+'T00:00:00');return d.getFullYear()===y&&d.getMonth()===m&&(activeCat==='Toutes'||(s.categories||[]).includes(activeCat))}).sort((a,b)=>(a.date+a.start).localeCompare(b.date+b.start))}
...
function renderSaisie(){const s=currentSession();currentTitle.textContent=s?`Séance du ${fmtDate(s.date)} · ${s.start||''}${s.end?' - '+s.end:''} · ${(s.categories||[]).join(' / ')} · ${s.theme||s.type}`:'Aucune séance sélectionnée';if(!s){playersBody.innerHTML='<tr><td colspan="6" class="empty">Crée une séance pour commencer.</td></tr>';renderKpis();return}const q=search.value.toLowerCase(),cf=catFilter.value,sf=statusFilter.value;let list=eligiblePlayers(s).filter(p=>(!cf||p.categorie===cf)&&(`${p.nom} ${p.prenom}`.toLowerCase().includes(q)));if(sf)list=list.filter(p=>normalizeCode(s.entries[p.id]?.code)===sf);playersBody.innerHTML=list.map(p=>{const e=s.entries[p.id]||{code:'P',minutes:s.duration,note:''};e.code=normalizeCode(e.code);const buttons=STATUS_ORDER.map(c=>`<button class="code-btn ${e.code===c?'active':''}" title="${CODES[c].label}" onclick="setCode('${p.id}','${c}')">${c}</button>`).join('');return `<tr><td><div class="player-name">${p.prenom} ${p.nom}</div><div class="muted">${p.annee}</div></td><td>${p.categorie||''}</td><td><span class="pill ${codeClass(e.code)}">${e.code}</span></td><td><div class="quick">${buttons}</div></td><td><input class="mini" type="number" value="${e.minutes||0}" min="0" onchange="setMinutes('${p.id}',this.value)"></td><td><input placeholder="note" value="${escapeHtml(e.note||'')}" onchange="setNote('${p.id}',this.value)"></td></tr>`}).join('')||`<tr><td colspan="6" class="empty">Aucune joueuse ne correspond aux catégories de cette séance : ${sessionSubCategories(s).join(', ')}</td></tr>`;renderKpis()}
...
function importJSON(ev){const f=ev.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{state=JSON.parse(r.result);state.sessions=(state.sessions||[]).map(s=>({...s,start:s.start||'',end:s.end||'',categories:s.categories||inferCats(s),entries:Object.fromEntries(Object.entries(s.entries||{}).map(([pid,e])=>[pid,{...e,code:normalizeCode(e.code)}]))}));saveNow();renderAll();renderCalendar()};r.readAsText(f)}
function exportCSV(){let rows=[['date','debut','fin','categories','type','theme','joueuse','categorie_joueuse','code','minutes','note']];state.sessions.forEach(s=>ROSTER.forEach(p=>{const e=s.entries[p.id]||{};rows.push([s.date,s.start||'',s.end||'',(s.categories||[]).join('|'),s.type,s.theme,`${p.prenom} ${p.nom}`,p.categorie,e.code||'',e.minutes||0,e.note||''])}));download('presence_seances_asse_v3_3.csv',rows.map(r=>r.map(x=>'"'+String(x).replaceAll('"','""')+'"').join(';')).join('\n'),'text/csv;charset=utf-8')}
function downloadEmptyCSV(){download('modele_presence_seances_v2.csv','date;debut;fin;categories;type;theme;joueuse;categorie_joueuse;code;minutes;note\n','text/csv')}
...
function renderFilters(){
  ...
  const cats=TRAINING_CATEGORIES;
  ...
  setOptions('bilanCatFilter','<option value="">Toutes catégories de séance</option>'+cats.map(c=>`<option value="${c}">${c}</option>`).join(''),old.bilanCat);
  ...
  setOptions('chargeCatFilter','<option value="">Toutes catégories de séance</option>'+cats.map(c=>`<option value="${c}">${c}</option>`).join(''),old.chargeCat);
  ...
  setOptions('prevCatFilter','<option value="">Toutes catégories de séance</option>'+cats.map(c=>`<option value="${c}">${c}</option>`).join(''),old.prevCat);
