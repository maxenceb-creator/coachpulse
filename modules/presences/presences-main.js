...
const presenceStorage=window.CoachPulsePresenceStorage;
let state=presenceStorage.load(INITIAL_EXCEL_STATE);
...
function localDateIso(date){return `${date.getFullYear()}-${pad2(date.getMonth()+1)}-${pad2(date.getDate())}`}
...
function saveNow(reason){presenceStorage.save(state,{reason:reason||'presences-module'})}
...
function createSessionObj(data){const id='s'+Date.now()+Math.floor(Math.random()*999);const session={id,...data,entries:{}};eligiblePlayers(session).forEach(p=>session.entries[p.id]={code:'P',minutes:data.duration,note:''});state.sessions.unshift(session);state.current=id;saveNow('create-session');renderAll();renderCalendar();renderDetail(state.sessions[0])}
function setDefaultRecurringUntil(){if(!window.recurUntil)return;const d=new Date();d.setMonth(d.getMonth()+3);recurUntil.value=localDateIso(d)}
...
function presenceCurrentSeason(){try{return window.parent?.CoachPulseCentralData?.currentSeason?.()||presenceSeasonFromDate(today())||'2026-2027'}catch(_){return presenceSeasonFromDate(today())||'2026-2027'}}
