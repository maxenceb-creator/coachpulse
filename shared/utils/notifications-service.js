...
(function(global){
  ...
  function ensureStyles(){
    ...
    style.textContent = `
      ...
      .cp-prompt-input{width:100%;margin-top:14px;border:1px solid rgba(0,63,36,.18);border-radius:14px;padding:11px 12px;background:#f8fafc;color:#06351f;font:800 14px/1.2 Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;outline:none}
      .cp-prompt-input:focus{border-color:#1d995b;box-shadow:0 0 0 3px rgba(29,153,91,.14);background:#fff}
  ...
  function prompt(message, options={}){
    const text = String(message || '').trim();
    if(!global.document || !text) return Promise.resolve(null);
    ensureStyles();
    return new Promise(resolve => {
      let settled = false;
      const backdrop = global.document.createElement('div');
      ...
      backdrop.setAttribute('role', 'dialog');
      backdrop.setAttribute('aria-modal', 'true');
      backdrop.innerHTML = `<section class="cp-confirm"><h2>${escapeHtml(options.title || 'Saisie requise')}</h2><p>${escapeHtml(text)}</p><input class="cp-prompt-input" data-prompt-input type="text" value="${escapeHtml(options.defaultValue || '')}" autocomplete="off"><div class="cp-confirm-actions"><button type="button" data-cancel>${escapeHtml(options.cancelLabel || 'Annuler')}</button><button type="button" class="${options.danger ? 'danger' : 'primary'}" data-confirm>${escapeHtml(options.confirmLabel || 'Valider')}</button></div></section>`;
      ...
      backdrop.addEventListener('click', event => {
        if(event.target === backdrop || event.target.closest('[data-cancel]')) close(null);
        if(event.target.closest('[data-confirm]')) close(input?.value || '');
      });
      global.document.addEventListener('keydown', onKey);
      global.document.body.appendChild(backdrop);
      input?.focus();
